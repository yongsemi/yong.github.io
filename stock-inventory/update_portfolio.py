...
TRACKER_PAGE  = '36c94a3e-d2ab-80f9-8b8e-fc7ff3efffcf'   # Portlifolio Tracker page
...
def update_tracker_page(holdings, fx):
    ...
    for blk in children:
        ...
        if btype == 'heading_2':
            ...
            # Only update the total-value paragraph (contains 'Total Portfolio')
            ...
            # Get table_row children and update each market row + total row
            rows = notion_get(f'/blocks/{bid}/children')['results']
            # rows[0] = header row (skip), rows[1]=US, rows[2]=TW, rows[3]=JP, rows[4]=Total
            mkt_order = ['US', 'TW', 'JP']
